...
(defun my-config/init-plantuml-mode()
  (use-package plantuml-mode
    ...
    :config
    (progn
      (defvar plantuml-indent-regexp-end "^[ \t]*\\(?:@enduml\\|\\(end\\ fork\\)\\|\\(fork\\ again\\)\\|endif\\|end\\|end\s+note\\|elseif\\|else\\|stop\\|}\\)")
      (defvar plantuml-indent-regexp-start "^[ \t]*\\(?:@startuml\\|\\(fork\\ again\\)\\|start\\|if\\|elseif\\|else\\|fork\\|\\(?:.*\\)?\s*\\(?:[<>.*a-z-|]+\\)?\s*\\(?:\\[[a-zA-Z]+\\]\\)?\s+if\\|note\s+over\\|note\s+\\(\\(?:\\(?:buttom\\|left\\|right\\|top\\)\\)\\)\\(?:\s+of\\)?\\|.*{\\)")
